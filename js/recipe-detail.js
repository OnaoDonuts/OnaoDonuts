...
recipe.steps.forEach((section, sIndex) => { // セクションの番号(sIndex)を追加
    ...
    // ★アドセンス対策用の長文コラム（column）を表示する処理
    const columnSection = document.getElementById('recipeColumnSection');
    const columnContent = document.getElementById('recipeColumn');
    
    if (columnSection && columnContent) {
        if (recipe.column) {
            columnContent.innerHTML = recipe.column;
            columnSection.style.display = 'block'; // コラムがある時はカードごと表示
        } else {
            columnSection.style.display = 'none';  // ない時はカードごと非表示
        }
    }
    
    // ▼ここから追記：導入文の「続きを読む」制御
    const readMoreBtn = document.getElementById('readMoreBtn');
    if (descriptionEl && readMoreBtn) {
        // 初期状態としてボタンを表示する
        readMoreBtn.style.display = 'block';

        // すでにクリックイベントが設定されている場合は一度削除して二重登録を防ぐ
        readMoreBtn.onclick = () => {
            descriptionEl.classList.toggle('open');
            if (descriptionEl.classList.contains('open')) {
